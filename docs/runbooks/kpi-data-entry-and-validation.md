...
事業上の基準タイムゾーンは、本リポジトリ内の既存文書で明示的に確定されていなかったため、独断で決定せず`not_decided`として扱う(下記「16. 現在の状態」参照)。外部サービスの表示タイムゾーンと事業報告タイムゾーンが異なる場合、変換前(`source_timezone`)と変換後(`reporting_timezone`)を区別する。
